...
class Actions extends P4A_Base_Mask
{
	public $toolbar = null;
	public $table = null;
	...
	public function __construct()
	{
		...
		$this->setSource($p4a->actions);
		$this->setTitle("Single activities - if a client contact leads to several planned activities, insert each activity here");
		$this->firstRow();
		...
		$this->fields->create_contact_id
			->setLabel("Client activity")
			->setType("select")
		...
		$this->build("p4a_table", "table")
			->setSource($p4a->actions)
			->setVisibleCols(array("contact","creation_time","description","deadline","status"))
			->setWidth(1200)
			->showNavigationBar();

		$this->build("p4a_fieldset", "fs_details")
			->setLabel("Action detail")
			->anchor($this->fields->create_contact_id)
			->anchor($this->fields->creation_time)
			->anchor($this->fields->description)
			->anchor($this->fields->details)
			->anchor($this->fields->security_id)
			->anchor($this->fields->deadline)
