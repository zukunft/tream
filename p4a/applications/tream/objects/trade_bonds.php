 ...
 */
class Trade_bonds extends P4A_Base_Mask
{
	public $toolbar = null;
	public $table = null;
	...
	public function __construct()
	{
		...
		$this->setTitle("Fixed income trades");
		...
		$this->build("p4a_db_source", "trades")
			...
			->setWhere("trades.security_id is Not Null AND v_securities.type_code_id = 'bond'")
			->setPageLimit(20)
			->load();
		...
		$this->fields->trade_type_id
			->setLabel("Trade type")
			->setType("select")
			->setSource(P4A::singleton()->trade_types_bond)
		...
		$this->fields->size->setLabel("Notional");
		$this->fields->price->setLabel("pct");
...
/*		$this->fields->internal_person_id
	...
	public function search()
	{
		...
		if ($value == '') {
			...
			$this->trades
				->setWhere(P4A_DB::singleton()->getCaseInsensitiveLikeSQL('v_securities.name', "%{$value}%")." AND v_securities.type_code_id = 'bond'")
				->firstRow();
			$this->security_bond
				->setWhere(P4A_DB::singleton()->getCaseInsensitiveLikeSQL('v_securities.name', "%{$value}%")." AND v_securities.type_code_id = 'bond'")
				->firstRow();
