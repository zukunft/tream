...
*/
class Users extends P4A_Base_Mask
{
	public $toolbar = null;
	public $table = null;
	...
	public function __construct()
	{
		...
		$this->setSource($p4a->users);
		$this->setTitle("TREAM users");
		$this->firstRow();
		...
		$this->fields->internal_person_id
			->setLabel("Person")
			->setTooltip("to link the contact details to the user")
			->setType("select")
		...
		$this->build("p4a_table", "table")
			->setSource($p4a->users)
			->setVisibleCols(array("username","type","comment"))
			->setWidth(700)
			->showNavigationBar();

		$this->setRequiredField("username");

		$this->build("p4a_fieldset", "fs_details")
			->setLabel("User detail")
			->anchor($this->fields->username)
