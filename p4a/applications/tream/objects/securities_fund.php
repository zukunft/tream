...
*/
class Securities_fund extends P4A_Base_Mask
{
	...
	public function __construct()
	{
		...
		$this->build("p4a_db_source", "securities")
			...
			->setWhere("(v_security_types.code_id = 'fund' OR v_security_types.code_id = 'ETF')") 
			->setPageLimit(30)
			->load();
		...
		$this->build("p4a_db_source", "security_type_funds")
		...
		$this->fields->security_type_id
			...
			->setType("select")
			->setSource($this->security_type_funds)
		...
		$this->build("p4a_table", "table")
			...
			->setVisibleCols(array("name","ISIN","valor","symbol_wikidata","symbol_yahoo","fx","last_price","update_time"))
			->setWidth(900)
			->showNavigationBar();
	...
	public function search()
	{
		...
		if (!$this->securities->getNumRows()) {
			...
			if (!$this->securities->getNumRows()) {
				...
				$this->securities->setWhere(null);
				$this->securities->firstRow();
			}
		}
	} 	
