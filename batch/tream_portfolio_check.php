...
// calculate (and display if requested) the exposure for one exposure item (e.g. the CHF exposure)
...
// calculate (and display if requested) the exposure for one exposure type e.g. for asset class or currency
// display any sub items first
// returns the aggregated exposure for double check
function exposure_by_type($portfolio_id, $exposure_type_id, $type_name, $total_aum, $ref_curr, $display) {
  ...
  // loop over the first level exposure items (e.g. cash, equity, bonds, ...) of the type and fill up the usage
...
function tream_check_portfolio_selector($portfolio_id, $display) {
  ...
  while ($portfolio_row = mysql_fetch_assoc($portfolio_result)) {
    if ($portfolio_row['portfolio_name'] <> "") {
        if ($portfolio_id == $portfolio_row['portfolio_id']) {
            tream_display ('<option selected value="'.$portfolio_row['portfolio_id'].'">'.$portfolio_row['portfolio_name'].'</option>', $display);
        } else {
...
function tream_check_portfolio($portfolio_id, $display) {
  ...
  // loop over the exposure types / exposure tree roots (e.g. asset classes, currencies, sectors)
